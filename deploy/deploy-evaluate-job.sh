#!/usr/bin/env bash
# Deploy evaluate-applications as a Cloud Run Job + daily Scheduler (9am America/New_York).
...
# Prerequisites:
#   gcloud auth login
#   Local .env filled (used to build deploy/evaluate-applications.env.yaml)
...
JOB_NAME="${JOB_NAME:-copilot-evaluate-applications}"
SCHEDULER_NAME="${SCHEDULER_NAME:-copilot-evaluate-applications-scheduler}"
REPOSITORY="${REPOSITORY:-cloud-run-source-deploy}"
IMAGE_NAME="${IMAGE_NAME:-copilot-evaluate-applications}"
...
# YAML for gcloud --env-vars-file (KEY: "value")
lines = []
for k in sorted(env):
...
echo "   Cron:      ${SCHEDULE} (${TIME_ZONE}) → 9:00am Eastern"
echo ""
